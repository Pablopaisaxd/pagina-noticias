...
// Noticias de ejemplo con URLs ficticias
const noticias = [
  ...
  {titulo: "Assassin's Creed Shadows será el primer juego desarrollado para la mitad del equipo, según Ubisoft", url:"https://vandal.elespanol.com/noticia/1350776136/assassins-creed-shadows-sera-el-primer-juego-desarrollado-para-la-mitad-del-equipo-segun-ubisoft/"}, // Sin enlace
...
lupa.addEventListener('click', () => {
  const textoBusqueda = busqueda.value.trim().toLowerCase(); // Convertir a minúsculas para búsqueda insensible a mayúsculas
  console.log('Texto de búsqueda:', textoBusqueda); // Verifica que el texto se esté leyendo correctamente

  if (textoBusqueda !== '') {
    // Filtrar noticias que contengan el texto ingresado
    const resultadosFiltrados = noticias.filter(noticia => 
      noticia.titulo.toLowerCase().includes(textoBusqueda) // Comparar noticia con el texto de búsqueda
    );

    console.log('Resultados filtrados:', resultadosFiltrados); // Verifica los resultados filtrados
    ...
    if (resultadosFiltrados.length > 0) {
      resultadosFiltrados.forEach((resultado) => {
        ...
        if (resultado.url) {
          // Si tiene URL, crear un enlace
          const enlace = document.createElement('a');  
          enlace.href = resultado.url; // Establecer la URL
          enlace.textContent = resultado.titulo;  // Establecer el texto del enlace
          elementoResultado.appendChild(enlace);  // Agregar enlace al div
